...
def mix_module(x0, x1, x2, C, k, p):
    # multi resolution
    x0 = grouped_attention_module(x0, C, k, p)
    x1 = grouped_attention_module(x1, C, k, p)
    x2 = grouped_attention_module(x2, C, k, p)
...
def create_model(input_shape=(40, 256, 1), output_dim=[144, 3]):
    ...
    x0, x1, x2 = mix_module(x0, x1, x2, C=32, k=1, p=3)
    # stage 2 (27, T, 32) -> (9, T, 64)
    x0, x1, x2 = mix_module(x0, x1, x2, C=64, k=1, p=3)
